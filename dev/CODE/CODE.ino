...
#define DIGITAL_OUT (1 << 0)
#define PWM_OUT (1 << 1)
#define ENABLE_OUT (1 << 2)
#define SPEED_OUT (1 << 3)
#define HOLD_DIGITAL (1 << 4)

uint8_t flags = 0 | (PWM_OUT);
...
void onConnectedController(ControllerPtr gamepad)
{
  if (Controllers[0] == nullptr)
  {
    ...
    flagger::remove(game_flags, BLINK);
    // game_flags &= ~BLINK;
...
float handleDirection(ControllerPtr gamepad, bool joystick)
{
  ...
  if (abs(direction) < 0.30)
    return 0.0;
  return fmap(direction, 0.30, 1.00, 0.00, 1.00);
}

void handleMovement(int PWM, float DIRECTION, bool INX)
{
#if flagger::has(flags, (PWM_OUT | SPEED_OUT)) // This only compiles if the flags has the flag for PWM Enable, aka: "Speed Out" or PWM Out per In X
...
#endif
#if flagger::has(flags, (DIGITAL_OUT | ENABLE_OUT)) // This only compiles if the flags has the flag for Digital Enable or Digital Out per In X
  bool DIGITAL_BASE_M1 = (M1_PWM > 0);
  bool DIGITAL_BASE_M2 = (M2_PWM > 0);
#endif

#if flagger::has(flags, SPEED_OUT) // This only compiles if the flags has the flag for PWM Enable, aka: "Speed Out"
  analogWrite(M1_PWM_SPEED, M1_PWM);
  analogWrite(M2_PWM_SPEED, M2_PWM);
#endif

#if flagger::has(flags, ENABLE_OUT) // This only compiles if the flags has the flag for Digital Enable
  digitalWrite(M1_ENABLE, DIGITAL_BASE_M1);
  digitalWrite(M2_ENABLE, DIGITAL_BASE_M2);
#endif

#if flagger::has(flags, DIGITAL_OUT)                   // This only compiles if the flags has the flag for Digital Output per In X
  const bool HOLD = flagger::has(flags, HOLD_DIGITAL); // This checks if the flags has the flag to hold the digital pins
  digitalWrite(!INX ? D_M1_IN1 : D_M1_IN2, false);
  digitalWrite(!INX ? D_M2_IN1 : D_M2_IN2, false);
  digitalWrite(INX ? D_M1_IN1 : D_M1_IN2, HOLD ? DIGITAL_BASE_M1 : !(DIRECTION < 0.0f));
  digitalWrite(INX ? D_M2_IN1 : D_M2_IN2, HOLD ? DIGITAL_BASE_M1 : !(DIRECTION > 0.0f));
#endif

#if flagger::has(flags, PWM_OUT) // This only compiles if the flags has the flag for PWM Output per In X
...
{
  if (!flagger::has(game_flags, SELECT_PRESSED) && gamepad->miscSelect())
  {
    flagger::add(game_flags, SELECT_PRESSED);
    flagger::toggle(game_flags, AXIS_LY);
  }
  if (flagger::has(game_flags, SELECT_PRESSED) && !gamepad->miscSelect())
  {
    flagger::remove(game_flags, SELECT_PRESSED);
  }
  if (!flagger::has(game_flags, START_PRESSED) && gamepad->miscStart())
  {
    flagger::add(game_flags, START_PRESSED);
    flagger::toggle(game_flags, AXIS_RX);
  }
  if (flagger::has(game_flags, START_PRESSED) && !gamepad->miscStart())
  {
    flagger::toggle(game_flags, START_PRESSED);
  }
}
...
void loop()
{
  ...
  {
    const int speed = handleDualSpeed(Controller, game_flags & AXIS_LY);
    checkConfig(Controller);

    if (speed != 0)
      handleMovement(abs(speed), handleDirection(Controller, game_flags & AXIS_RX), speed > 0);
    else
    {
      handleMovement(0, 0.0, false);
      handleMovement(0, 0.0, true);
